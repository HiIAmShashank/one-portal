...
export const shellEnvSchema = z.object({
  ...
  VITE_MSAL_AUTHORITY: z.string().url('MSAL Authority must be a valid URL'),
  VITE_MSAL_REDIRECT_URI: z.string().url('Redirect URI must be a valid URL').optional(),
...
export const remoteEnvSchema = z.object({
  VITE_REMOTE_NAME: z.string().min(1, 'Remote name is required'),
  VITE_API_BASE_URL: z.string().url('API Base URL must be a valid URL').optional(),
});
