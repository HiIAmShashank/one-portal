// apps/remote-billing/src/auth/MSALProvider.tsx
...
interface BillingMSALProviderProps {
  children: ReactNode;
}

/**
 * Billing MSAL Provider
 * - Attempts silent SSO on mount
 * - Redirects to Shell if authentication fails
 * - Listens for auth events from Shell
 */
export function BillingMSALProvider({ children }: BillingMSALProviderProps) {
      ...
      try {
        ...
        if (accounts.length > 0) {
          ...
          try {
            ...
            try {
              ...
              if (document.visibilityState === 'visible') {
                console.error('[Billing] SSO failed, redirecting to Shell:', ssoError);
                const returnUrl = encodeURIComponent(window.location.href);
                window.location.href = `/?returnUrl=${returnUrl}`;
              } else {
                // App is being preloaded, just mark as initialized
                if (import.meta.env.DEV) {
                  console.log('[Billing] SSO failed (preload), skipping redirect');
                }
                setIsInitialized(true);
              }
            }
          ...
          try {
            ...
            if (document.visibilityState === 'visible') {
              console.error('[Billing] No session, redirecting to Shell:', error);
              const returnUrl = encodeURIComponent(window.location.href);
              window.location.href = `/?returnUrl=${returnUrl}`;
            } else {
              // App is being preloaded, just mark as initialized
              if (import.meta.env.DEV) {
                console.log('[Billing] No session (preload), skipping redirect');
              }
              setIsInitialized(true);
            }
          }
  ...
  useEffect(() => {
    const unsubscribe = subscribeToAuthEvents(async (event) => {
      if (event.type === 'auth:signed-in' && event.payload?.loginHint) {
        ...
        if (import.meta.env.DEV) {
          console.log('[Billing] Received auth:signed-out event, clearing cache');
        }
  ...
  useEffect(() => {
    const callbackId = msalInstance.addEventCallback((event: any) => {
      if (event.eventType === MsalEventType.LOGOUT_SUCCESS) {
        console.log('[Billing] Logged out');
      }
    });
