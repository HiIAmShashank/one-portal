// apps/remote-testgenerator1/src/auth/MSALProvider.tsx
...
interface Testgenerator1MSALProviderProps {
  children: ReactNode;
}

/**
 * Test Gen 1 MSAL Provider
 * 
 * Features:
 ...
 */
export function Testgenerator1MSALProvider({ children }: Testgenerator1MSALProviderProps) {
      ...
      try {
        ...
        if (accounts.length > 0) {
          ...
          try {
            ...
            announceToScreenReader('Test Gen 1 authenticated successfully');
          } catch (error: any) {
            ...
            try {
              ...
              setIsInitialized(true);
              announceToScreenReader('Test Gen 1 authenticated via SSO');
            } catch (ssoError: any) {
              console.error('[Test Gen 1] SSO failed:', ssoError);
        ...
        console.error('[Test Gen 1] Auth initialization failed:', error);
  ...
  useEffect(() => {
    const unsubscribe = subscribeToAuthEvents(async (event) => {
      if (event.type === 'auth:signed-in') {
        console.log('[Test Gen 1] Received auth:signed-in event from Shell');
        ...
        try {
          ...
          msalInstance.setActiveAccount(ssoResult.account);
          setIsInitialized(true);
          announceToScreenReader('Test Gen 1 signed in via SSO');
          console.log('[Test Gen 1] SSO successful');
        } catch (error) {
          console.error('[Test Gen 1] SSO failed after Shell sign-in:', error);
          showAuthError(error, undefined, { announceToScreenReader });
        }
      } else if (event.type === 'auth:signed-out') {
        console.log('[Test Gen 1] Received auth:signed-out event from Shell');
        await msalInstance.clearCache();
        msalInstance.setActiveAccount(null);
        setIsInitialized(false);
        announceToScreenReader('Test Gen 1 signed out');
      }
    });
  ...
  useEffect(() => {
    const callbackId = msalInstance.addEventCallback((event) => {
      if (event.eventType === MsalEventType.LOGIN_SUCCESS && event.payload) {
        ...
        setIsInitialized(true);
        announceToScreenReader('Test Gen 1 sign-in successful');
      }
    });
