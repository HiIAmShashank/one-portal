...
export class AuthErrorHandler {
    ...
    static process(error: unknown, context?: string): ProcessedAuthError {
        ...
        if (error instanceof InteractionRequiredAuthError) {
            return {
                ...
                message: context ? `${context}: ${error.errorMessage}` : error.errorMessage,
    ...
    private static processBrowserAuthError(
        ...
        const message = context ? `${context}: ${error.errorMessage}` : error.errorMessage;
        const baseError = {
            code: error.errorCode,
            message,
            subError: error.subError,
    ...
    static show(
        ...
        const toastOptions = {
            ...
            ...(processed.isRetryable && options?.onRetry && {
                action: {
