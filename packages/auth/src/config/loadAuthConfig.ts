...
export function loadAuthConfig(appName: string): AuthConfig {
  const clientId = import.meta.env[`${ENV_PREFIX}CLIENT_ID`] as
    | string
    | undefined;
  const authority = import.meta.env[`${ENV_PREFIX}AUTHORITY`] as
    | string
    | undefined;
  const redirectUri = import.meta.env[`${ENV_PREFIX}REDIRECT_URI`] as
    | string
    | undefined;
  const postLogoutRedirectUri = import.meta.env[
    `${ENV_PREFIX}POST_LOGOUT_REDIRECT_URI`
  ] as string | undefined;
  const scopesStr = import.meta.env[`${ENV_PREFIX}SCOPES`] as
    | string
    | undefined;
